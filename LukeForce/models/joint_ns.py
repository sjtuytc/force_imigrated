...
class JointNS(BaseModel):
    ...
    def resnet_features(self, x):
        def forward_x(ten):
            ...
            ten = self.feature_extractor.conv1(ten)
            ten = self.feature_extractor.bn1(ten)
            ten = self.feature_extractor.relu(ten)
            ten = self.feature_extractor.maxpool(ten)

            ten = self.feature_extractor.layer1(ten)
            ten = self.feature_extractor.layer2(ten)
            ten = self.feature_extractor.layer3(ten)
            ten = self.feature_extractor.layer4(ten)
    ...
    def get_optim(self):
        if self.joint_two_losses:
            return self.joint_optim
        else:
            return self.loss1_optim
