...
class PhysicEnvTester(nn.Module):
    def __init__(self, args, object_paths, object_name, context, nproc, nenvs, parallel=True):
        ...
        if parallel:
            self.environment = SubprocPhysicsEnv(args=args, object_paths=object_paths, context=context, nproc=nproc, nenvs=nenvs)
...
def main():
    ...
    force_data = np.array([[-0.0047, -0.0841, -0.0801] for i in range(5)])
    ...
    env_state, force_success, force_applied = test_m(env_state_tensor=initial_state,
